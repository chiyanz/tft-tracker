...
export default function Player({ data }: { data: PlayerInfo }) {
    ...
    if (!ranked) {
      ...
      return (
        <>
          ...
          < img
            src={rankedImages[`${ranked.tier}.png`].default.src}
  ...
  const placementsDisplay = placementHistory.map((place, i) => {
    return (
  ...
  return (
    <>
      ...
      <div className={styles.stats}>
        {placementHistory && (
          <div>
            <div className="text-lg">Recent 20 Matches</div>
            <div className={styles.matchhistory}>{placementsDisplay}</div>
          </div>
        )}
        {stats && (
